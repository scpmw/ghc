...
collect str = collect2 str >>= return . fst

-- | Turns a Stream into an ordinary list as well as a return value
collect2 :: Monad m => Stream m a x -> m ([a],x)
collect2 str = go str []
 where
  go str acc = do
    r <- runStream str
