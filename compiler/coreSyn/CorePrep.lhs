...
wrapTicks (Floats flag floats0) expr
    = (mapFloats (tickIt mkNoTick) (Floats flag (toOL floats)),
       tickIt id expr)
  where getTick (FloatTick t) = Left t
        getTick f             = Right f
        (ticks, floats) = partitionWith getTick $ fromOL floats0
        tickIt f e = foldr (Tick . f) e ticks

mapFloats :: (CoreExpr -> CoreExpr) -> Floats -> Floats
mapFloats f (Floats flag floats) = Floats flag (mapOL wrap floats)
  where wrap (FloatLet bind)    = FloatLet (wrapBind bind)
        wrap (FloatCase b r ok) = FloatCase b (f r) ok
        wrap other              = pprPanic "unfloatTicks: unexpected float!" (ppr other)
        wrapBind (NonRec binder rhs) = NonRec binder (f rhs)
        wrapBind (Rec pairs)         = Rec (mapSnd f pairs)
