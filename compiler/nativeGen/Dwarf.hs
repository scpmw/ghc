...
debugFrameHeader u
  = sdocWithPlatform $ \plat ->
    let cieLabel    = mkAsmTempLabel u
        ...
        toRegNo r   = maybe 0 (dwarfRegNo plat . RegReal) $ globalRegMaybe plat r
        spReg       = toRegNo Sp
    ...
    in vcat [ ppr cieLabel <> colon          -- Label to refer to this CIE
            ...
            , pprData4' $ ptext (sLit "-1")  -- Common Information Entry marker
...
-- instructions to describe the blocks (see @debugBlockFrame@)
...
-- have seen no code trying to correct this assumption.
--
